...
const createCart = async (req, res) => {
    try {
        ...
        if (!cardId) {
            let cartExistforUser = await CartModel.findOne({ userId:data.userId })
            if (cartExistforUser) {
        ...
        const requiredParams = ['userId', 'items','cardId'];
        ...
        if (fetchCart) {
           ...
           let CartExist = await CartModel.findOne({ _id: cardId });
           if (!CartExist) {
                   return res.status(403).send({ status: false, message: "This cart does not belong to you. Please check the cart Id" })
           }
           if(Object.keys(data).length==2){
            return res.status(400).send({ status: false, message: "please add a Product" })
           }
            let status = false;
            const previousItems = [];
            ...
            if(data['cardId']!=undefined){
                return res.status(400).send({status: true,message: "card is not created yet,please don't give ramdom cardId"});
            }
           ...
           if(data.removeProduct!=undefined){
            ...
            if (data.removeProduct == 0 || data.removeProduct == 1) {
                ...
                if (data.removeProduct == 0) {
                    ...
                    return res.status(200).send({status: true,message: "success",data: reduceRes});
                }
            }
            else {
